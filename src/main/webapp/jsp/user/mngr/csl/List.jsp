...
<form:form commandName="mngrUserSearchVO" id="defaultForm" method="get" action="list.do" >
${mngrUserSearchVO.superHiddenTagWithoutToggle}
		...
		<fieldset>
			...
			<ul>
				<li>
					...
					<input type="text" id="searchStartFrstOperDt" name="searchStartFrstOperDt" value="${mngrUserSearchVO.searchStartFrstOperDt}" class="validate[custom[date]] input_box datepicker" style="width:75px;" maxlength="10" />
					~
					<input type="text" id="searchEndFrstOperDt" name="searchEndFrstOperDt" value="${mngrUserSearchVO.searchEndFrstOperDt}" class="validate[custom[date]] input_box datepicker" style="width:75px" maxlength="10" />
					
					<label class="w_none" for="searchRegSt">관리자상태</label>
					...
					<label class="w_none" for="searchAuthId">관리자권한</label>
					...
					<form:input path="searchValue" cssClass="input_box" title="검색어" size="30" style="width:170px;" />
		...
		<fieldset>
			...
			<ul>
				<li class="btn_wrap">
					...
					<span class="result">검색결과 ( <fmt:formatNumber value="${mngrUserSearchVO.totalRecordCount}" />건  )</span>
					<p class="btn">
...
<table cellpadding="0" class="tb_base" summary="회원정보 테이블입니다.">
	...
	<tbody class="list">
	<c:forEach var="item" items="${mngrUserList}" varStatus="i">
		<tr>
			<td>${i.count }</td>
			<td>${item.userNm }</td>
			<td>${item.userId }</td>
			...
			<td>
				<!-- fmt:formatDate pattern="yyyy-MM-dd" value="${item.lastLoginDt}" /-->
				<fmt:parseDate value='${item.lastLoginDt}' var='lastLoginDt' pattern='yyyymmdd'/> <!-- J's 20230202 -->
				<fmt:formatDate value="${lastLoginDt}" pattern="yyyy-mm-dd"/>
			</td>
			<td>
...
<!-- 페이징 -->
<ui:pagination paginationInfo="${mngrUserSearchVO }" type="cslDefault" jsFunction="cfnPageLink" />
<!-- //페이징 -->
