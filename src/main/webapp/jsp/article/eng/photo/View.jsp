	...
	<div class="file_show">
		<c:if test="${boardVO.attachFlg eq 'Y'}">
			<c:if test="${not empty articleVO.fileList }">
				<div class="file_show">
					<p>
						File ${fn:length(articleVO.fileList)}
		...
		<p class="ta_r">
			<c:if test="${boardVO.replyFlg eq 'Y'}">
				<c:if test="${isUseGrpForReply}">
					<a href="#none" class="at_btn_st2" onclick="return replyForm();">Reply</a>
				</c:if>
			</c:if>
			<c:if test="${isUseGrpForWrite}">
				<a href="#none" class="at_btn_st2" onclick="return updateForm();">Modify</a>
				<a href="#none" class="at_btn_st2" onclick="return disable();">Delete</a>			
			</c:if>
			<a href="#none" class="at_btn_st2" onclick="list();">List</a>
		</p>
		<!-- //버튼 -->
	</div>
...
<div id="article_header">
	...
	<div id="article_sel">
		...
		<form:select path="searchClass" cssClass="sel_type ml15" cssStyle="width:80px;">
			...
			<form:option value="WRITER" label="Writer" />
		</form:select>
						...
						<a href="#none" onclick="view('${item.articleKey}');" title="${item.title }">
							<c:choose>
								<c:when test="${not empty item.fileList}">
									<c:choose>
										<c:when test="${item.fileList[0].image}">
											<img style="width:100px;" src="/attachfile/previewImage.do?fileId=${item.fileList[0].fileId}&amp;fileSeq=${item.fileList[0].fileSeq}" alt="${item.title }" />
										</c:when>
										<c:otherwise>
											<img style="width:100px;" src="/def/img/article/noimg.png" alt="이미지가 없습니다" />
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:otherwise>
