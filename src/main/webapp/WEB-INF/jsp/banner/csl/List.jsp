...
//excel
function excel(){
	$("#defaultForm").attr('target', 'downloadFrame');
	$("#defaultForm").attr('action', 'excel.do');
	$("#defaultForm").submit();
	return false;
}
//-->
...
<form:form commandName="bannerSearchVO" id="defaultForm" method="get" action="list.do" >
${bannerSearchVO.superHiddenTagWithoutToggle}

<input type="hidden" id="bannerKey" name="bannerKey" value="" />
		...
		<fieldset>
			...
			<ul>
				<li>
					...
					<input type="text" id="searchStartFrstOperDt" name="searchStartFrstOperDt" value="${bannerSearchVO.searchStartFrstOperDt}" class="validate[custom[date]] input_box datepicker" style="width:75px;" maxlength="10" />
					~
					<input type="text" id="searchEndFrstOperDt" name="searchEndFrstOperDt" value="${bannerSearchVO.searchEndFrstOperDt}" class="validate[custom[date]] input_box datepicker" style="width:75px" maxlength="10" />
					...
					<form:select path="searchLocateTyp">
						<form:option value="" label="전체" />
						<form:option value="DEF_BNR_01" label="하단배너" />
						<form:option value="DEF_BNR_02" label="좌측배너" />
					</form:select>
		...
		<fieldset>
			...
			<ul>
				<li class="btn_wrap">
					...
					<span class="result">검색결과 ( <fmt:formatNumber value="${bannerSearchVO.totalRecordCount}" />건  )</span>
					<p class="btn">
						<button class="jCreateButton" onclick="excel();">액셀다운로드</button>
...
<table cellpadding="0" class="tb_base" summary="등록하신 링크의 이미지,제목,주소,새창여부,활성화여부,위치변경여부,수정일 및 수정 및 삭제 여부테이블 입니다.">
	...
	<tbody class="list">
		...
		<tr>
			<td>
				${bannerSearchVO.totalRecordCount-((bannerSearchVO.currentPageNo-1)*bannerSearchVO.recordCountPerPage)-(i.count-1) }
			</td>
			<td style="text-align:center;">
				<c:choose>
					<c:when test="${item.locateTyp eq 'MAIN_VISUAL' }">메인비주얼배너</c:when>
					<c:when test="${item.locateTyp eq 'MAIN_BOTTOM' }">메인하단배너</c:when>
					<c:when test="${item.locateTyp eq 'SUB_VISUAL' }">서브비주얼배너</c:when>
				</c:choose>
			</td>
			<td>
				<c:if test="${not empty item.fileMsk }">
					<img src="<c:url value='/upload/banner/${item.fileMsk }'/>" alt="${item.title }" width="110px" height="30px" />
				</c:if>
			</td>
			...
			<td>
				<button class="jButton" onclick="updateForm('${item.bannerKey}');">수정</button>
				<button class="jButton" onclick="return remove('${item.bannerKey}');">삭제</button>
			</td>
		</tr>
		</c:forEach>
...
<!-- 페이징 -->
<ui:pagination paginationInfo="${bannerSearchVO }" type="cslDefault" jsFunction="cfnPageLink" />
<!-- //페이징 -->
