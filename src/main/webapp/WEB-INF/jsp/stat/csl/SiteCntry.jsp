...
function statAll(statTyp, year){
	$('#chartCntryWrap').css('display', 'none');
...
function statCntry(statTyp, year){
	$('#chartCntryWrap').css('display', '');
	$('#chartYearWrap').css('display', 'none');
	$('#chartMonthWrap').css('display', 'none');
	$('#chartDateWrap').css('display', 'none');
	$('#chartHourWrap').css('display', 'none');
	$('#chartMinuteWrap').css('display', 'none');
	$('#chartCntryWrap').empty().append("<canvas id=\"chartCntry\" width=\"600\" height=\"150\"></canvas>");
	$.ajax({
		url : '/csl/stat/getStatCntry.do' 
        ,method : "post"
		,dataType : 'json'
		,data : {
			statTyp : statTyp
		...
		,success : function(data, stat, xhr) {
			if(data != null && data.statSize > 0){
				console.log(data)
				window.chartCntry = new Chart(document.getElementById("chartCntry").getContext("2d"), {
				    type: 'bar'
				    ,options: {
				    	scales: {
				            yAxes: [{stacked: true}]
			...
			$('#chartCntry').bind('click', function(evt){
				var activePoints = chartCntry.getElementsAtEvent(evt);
	            var firstPoint = activePoints[0];
	            if(firstPoint != null){
	            	//console.log(chartCntry.data.labels[tooltipItem[0].index]);
		            var label = chartCntry.data.labels[firstPoint._index];
		            var value = chartCntry.data.datasets[firstPoint._datasetIndex].data[firstPoint._index];
...
/*
function statYear(statTyp, statCntry){
	console.log(statCntry);
		...
		,success : function(data, stat, xhr) {
			...
			$('#chartYear').bind('click', function(evt){
	            ...
	            if(firstPoint != null){
		            ...
		            //console.log(label + ": " + value);
		            statMonth(statTyp, statCntry, label.replace(/\D/g,''));
	            }
			});
		}
		,complete : function(){
...
function statMonth(statTyp, statCntry, year){
		...
		,success : function(data, stat, xhr) {
			...
			$('#chartMonth').bind('click', function(evt){
	            ...
	            if(firstPoint != null){
		            ...
		            statDate(statTyp, statCntry, year, label.replace(/\D/g,''));
	            }
			});
		}
		,complete : function(){
...
function statDate(statTyp, statCntry, year, month){
		...
		,success : function(data, stat, xhr) {
			if(data != null && data.statSize > 0){
				...
				window.chartYMD = new Chart(document.getElementById("chartDate").getContext("2d"), {
				    ...
				    ,data: {
					    ...
					    ,datasets: [
					        {
					            label: year + "년 " + month + "월, " + statCntry + ", 일별통계"
			...
			$('#chartDate').bind('click', function(evt){
	            ...
	            if(firstPoint != null){
		            ...
		            statHour(statTyp, statCntry, year, month, label.replace(/\D/g,''));
	            }
			});
		}
		,complete : function(){
...
function statHour(statTyp, statCntry, year, month, date){
		...
		,success : function(data, stat, xhr) {
			if(data != null && data.statSize > 0){
				...
				window.chartYMDH = new Chart(document.getElementById("chartHour").getContext("2d"), {
				    ...
				    ,data: {
					    ...
					    ,datasets: [
					        {
					            label: year + "년 " + month + "월 " + date + "일, " + statCntry + ", 시간별통계"
			...
			$('#chartHour').bind('click', function(evt){
	            ...
	            if(firstPoint != null){
		            ...
		            statMinute(statTyp, statCntry, year, month, date, label.replace(/\D/g,''));
	            }
			});
		}
		,complete : function(){
...
function statMinute(statTyp, statCntry, year, month, date, hour){
			...
			if(data != null && data.statSize > 0){
				...
				window.chartYNDHM = new Chart(document.getElementById("chartMinute").getContext("2d"), {
				    ...
				    ,data: {
					    ...
					    ,datasets: [
					        {
					            label: year + "년 " + month + "월 " + date + "일 " + hour + "시, " + statCntry + ", 분별통계"
					            ,backgroundColor: backgroundColor[0]
					            ,borderColor: borderColor[0]
					            ,borderWidth: 1
					            ,data: data.statData
...
<div id="chartCntryWrap" style="display:none;width:100%;"></div>
<div style="height:60px;"></div>
<!--  
