...
/* 			proposalInfo.PROP_NAME = $('#input-proposal-name').val().trim(); //제안명
			proposalInfo.PROP_CATEGORY_CODE = $('#select-category-code').val(); //제안유형
			proposalInfo.TYPE_CODE = 'PP_TY_1'; // 실시제안
			proposalInfo.PROP_PROBLEM = $('#textarea-proposal-problem').val().trim(); // 현상 및 문제점
			proposalInfo.PROP_IMPROVEMENT_CONST = $('#textarea-proposal-enhance').val().trim(); // 개선내용
			proposalInfo.PROP_BENEFIT = $('#textarea-proposal-effect').val().trim(); // 기대효과
			proposalInfo.PROP_YEAR_EFFECT_CODE = $('#select-yeareffect-code').val(); // 연간효과금액코드
			proposalInfo.PROP_YEAR_EFFECT = $('#input-yeareffect-remark').val().trim();
			//관련쪽지 여부
			if($('#input-proposal-memo-hidden').val() == '' && $('#input-proposal-memo').val() == ''){
				proposalInfo.PROP_REL_MEMO_YN = 'N';
			}else{
				proposalInfo.PROP_REL_MEMO_YN = 'Y';
				proposalInfo.PROP_REL_MEMO_CODE = $('#input-proposal-memo-hidden').val();
			}
			
			proposalInfo.PROP_GROUP_NAME = $('#text-proposal-group').val().trim(); //제안부서
			proposalInfo.PROP_BELT_CODE = $('#input-proposal-belt-hidden').val(); //벨트코드
			proposalInfo.PROP_BIZ_PLACE_CODE = $('#input-proposal-bizplace-hidden').val(); //사업장코드
			proposalInfo.PROP_DATE = $('#input-proposal-date').val(); //제안일
			proposalInfo.PROP_PRACTICE_COMP_DATE = $('#input-complete-date').val(); //실사완료일
			proposalInfo.PROP_REG_USER = $('#input-proposal-hidden').val(); //제안자
			proposalInfo.PROP_PROP_STAT_CODE = 'IP' // 제안상태 코드 - 입력중 (IP)
