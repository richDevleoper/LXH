        ...
        function getParentNode ( _children, item ) {
            ...
            for ( var i=0, child; child = _children[i]; i++ ) {
                ...
                if ( child[setting.id] == item[setting.parentId] ) {
					...
					{
						if(setting.order.length == 2){
							if(setting.order[1].toLowerCase() == "asc"){
								return a[setting.order[0]] < b[setting.order[0]] ? -1 : a[setting.order[0]] > b[setting.order[0]] ? 1 : 0;
							}else{
								return a[setting.order[0]] > b[setting.order[0]] ? -1 : a[setting.order[0]] < b[setting.order[0]] ? 1 : 0;
							}
						}

						if(setting.order.length == 4){
							if(a[setting.order[0]] == b[setting.order[0]]){
								if(setting.order[3].toLowerCase() == "asc"){
									return a[setting.order[2]] < b[setting.order[2]] ? -1 : a[setting.order[2]] > b[setting.order[2]] ? 1 : 0;
								}else{
									return a[setting.order[2]] > b[setting.order[2]] ? -1 : a[setting.order[2]] < b[setting.order[2]] ? 1 : 0;
								}
							}else{
	...
	$.fn.zooTree = function (data, setting) {
		...
		function createTreeItem(jsonData, $ul) {
			...
			if(jsonData) {
				...
				if(jsonData[setting.children] && jsonData[setting.children].length) {
					if(jsonData[setting.children] && jsonData[setting.children].length) {
						var $innerUl = $("<ul></ul>");
						$(jsonData[setting.children]).each(function(){
