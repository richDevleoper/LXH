...
function checkUserId(){
	var _result = false;
	$.ajax({
		url : '/csl/user/mngr/checkUserId.do'
        ,method : "post"
		,dataType : 'json'
		,async: false
...
<body>
<form:form commandName="mngrUserVO" id="defaultForm" name="defaultForm" method="post" action="${action}">
${mngrUserSearchVO.hiddenTag }

<table cellpadding="0" class="tb_base" summary="">
	<caption><span>관리자정보</span></caption>
	<colgroup>
		<col style="width:15%"/>
		<col style="width:85%"/>
	</colgroup>
	<thead>
		<tr>
			<th scope="col" colspan="2">관리자정보</th>
		</tr>
	</thead>
	<tbody class="write">
			...
			<td>
				<select id="authId" name="authId" class="validate[required] sel_type" title="권한">
					...
					<c:forEach var="item" items="${userAuthList}">
						<option value="${item.authId}" <c:if test="${mngrUserVO.authId eq item.authId}">selected="selected"</c:if>>${item.authNm}</option>
					</c:forEach>
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<c:choose>
					<c:when test="${not empty mngrUserVO.userId}">
