...
<form:form commandName="corpUserSearchVO" id="defaultForm" method="get" action="list.do" >
${corpUserSearchVO.superHiddenTagWithoutToggle}
		...
		<fieldset>
			...
			<ul>
				<li>
					...
					<input type="text" id="searchStartFrstOperDt" name="searchStartFrstOperDt" value="${corpUserSearchVO.searchStartFrstOperDt}" class="validate[custom[date]] input_box datepicker" style="width:75px;" maxlength="10" />
					~
					<input type="text" id="searchEndFrstOperDt" name="searchEndFrstOperDt" value="${corpUserSearchVO.searchEndFrstOperDt}" class="validate[custom[date]] input_box datepicker" style="width:75px" maxlength="10" />
					...
					<form:select path="searchClass">
						...
						<form:option value="CHR_TEL" label="담당자 전화번호" />
						<form:option value="CHR_MOB" label="담당자 휴대폰번호" />
						<form:option value="CHR_EMAIL" label="담당자 이메일" />
					</form:select>
					<form:input path="searchValue" cssClass="input_box" title="검색어" size="30" style="width:140px;" />
		...
		<fieldset>
			...
			<ul>
				<li class="btn_wrap">
					...
					<span class="result">검색결과 ( <fmt:formatNumber value="${corpUserSearchVO.totalRecordCount}" />건  )</span>
					<p class="btn">
...
<table cellpadding="0" class="tb_base" summary="회원정보 테이블입니다.">
	...
	<tbody class="list">
	<c:forEach var="item" items="${corpUserList}" varStatus="i">
		<tr>
			<td>${i.count }</td>
			<td>${item.corpNm }</td>
			<td>${item.userId }</td>
...
<!-- 페이징 -->
<ui:pagination paginationInfo="${corpUserSearchVO }" type="cslDefault" jsFunction="cfnPageLink" />
<!-- //페이징 -->
